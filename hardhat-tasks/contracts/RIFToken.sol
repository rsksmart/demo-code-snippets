...
library AddressHelper {
    ...
    function getHashedPublicKey(bytes32 _xPoint, bytes32 _yPoint)
        public
        pure
        returns (bytes20 hashedPubKey)
    {
        ...
        return
            ripemd160(
                abi.encodePacked(
                    sha256(abi.encodePacked(startingByte, _xPoint, _yPoint))
                )
            );
    }

    function fromHexChar(uint256 c) public pure returns (uint256) {
        if (c >= uint256(bytes1('0')) && c <= uint256(bytes1('9'))) {
            return c - uint256(bytes1('0'));
        }

        if (c >= uint256(bytes1('a')) && c <= uint256(bytes1('f'))) {
            return 10 + c - uint256(bytes1('a'));
        }
    ...
    function fromAsciiString(string s) public pure returns (address) {
        ...
        if (ss.length == 42) {
            ...
            if (ss[0] != bytes1('0')) revert();
            if (ss[1] != bytes1('x') && ss[1] != bytes1('X')) revert();
        }
...
library AddressLinker {
    ...
    function acceptLinkedRskAddress(
        ...
        address recoveredAddress = AddressHelper.recoverAddress(
            hash,
            sig_v,
            sig_r,
            sig_s
...
contract RIF is DetailedERC20, Ownable, StandardToken {
    /**
    ...
    mapping(address => uint256) public minimumLeftFromSale;
    ...
    constructor() public DetailedERC20('RIF', 'RIF', 18) {
    ...
    function getMinimumLeftFromSale(address a) public view returns (uint256) {
    ...
    function closeTokenDistribution(uint256 _distributionTime)
        public
        onlyAuthorizedManagerContract
    {
    ...
    function transferToShareholder(address wallet, uint256 amount)
        public
        onlyWhileInDistribution
        onlyAuthorizedManagerContract
    {
    ...
    function transferToContributor(address contributor, uint256 amount)
        public
        onlyWhileInDistribution
        onlyAuthorizedManagerContract
    {
    ...
    function delegate(address from, address to)
        public
        onlyAuthorizedManagerContract
        returns (bool)
    {
    ...
    function redeem(
        ...
        address redeemAddress = AddressHelper.fromAsciiString(
            redeemAddressAsString
        );
        ...
        require(
            AddressLinker.acceptLinkedRskAddress(
                contributorAddress,
                chainId,
                redeemAddressAsString,
                sig_v,
        ...
        minimumLeftFromSale[redeemAddress] = minimumLeftFromSale[
            contributorAddress
        ];
    ...
    function transfer(address _to, uint256 _value)
        public
        onlyAfterDistribution
        onlyIfAddressUsable(msg.sender)
        returns (bool)
    {
    ...
    function transferFrom(
        ...
        address _to,
        uint256 _value
    ) public onlyAfterDistribution onlyIfAddressUsable(_from) returns (bool) {
        // cannot transfer to a redirected account
        if (wasRedirected(_to)) return false;
    ...
    function doTrackMinimums(address addr) private {
        ...
        uint256 m = minimumLeftFromSale[addr];
